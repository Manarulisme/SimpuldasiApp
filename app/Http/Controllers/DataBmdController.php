...
class DataBmdController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($sync) {
            ...
            return redirect()
                ->route('databmd.index')
                ->with('success', 'Data BMD berhasil ditambahkan dan disinkronkan ke Google Sheets.');
        }
        ...
        return redirect()
            ->route('databmd.index')
            ->with('warning', 'Data BMD berhasil disimpan, tetapi gagal disinkronkan ke Google Sheets.');
    }
    ...
    public function update(Request $request, DataBmd $databmd)
    {
        ...
        if ($sync) {
            ...
            return redirect()
                ->route('databmd.index')
                ->with('success', 'Data BMD berhasil diperbarui dan disinkronkan ke Google Sheets.');
        }
        ...
        return redirect()
            ->route('databmd.index')
            ->with('warning', 'Data BMD berhasil diperbarui, tetapi gagal disinkronkan ke Google Sheets.');
    }
    ...
    public function destroy(DataBmd $databmd)
    {
        ...
        if (!$sync) {
            return redirect()
                ->route('databmd.index')
                ->with('error', 'Data BMD tidak dapat dihapus karena gagal menghapus data di Google Sheets.');
        }
        ...
        return redirect()
            ->route('databmd.index')
            ->with('success', 'Data BMD berhasil dihapus.');
    }
    ...
    private function sendToGoogleSheets(array $data): bool
    {
        ...
        if (!$url) {
            Log::error('GOOGLE_SHEETS_WEBHOOK_URL tidak ditemukan di .env');
        ...
        if ($jsonData === false) {
            Log::error('Gagal encode JSON Google Sheets BMD', [
                'data' => $data,
        ...
        Log::info('Response Google Sheets BMD - Initial', [
            'http_code' => $httpCode,
         ...
         *
         * Kita tidak POST ulang ke URL redirect.
         * URL redirect dibuka menggunakan GET.
         */
        if (
            ($httpCode === 301 || $httpCode === 302 || $httpCode === 303)
            && $redirectUrl
        ) {
            ...
            Log::info('Response Google Sheets BMD - Redirect', [
            ...
            if ($finalResponse === false || $finalCurlErrno !== 0) {
                return false;
            }
        ...
        /*
         * Jika Google tidak memberikan redirect tetapi
         * langsung memberikan JSON response.
         */
